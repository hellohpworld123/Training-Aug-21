...
-- Join
--Join Clause use to retrive collective data from more the one related tables
...
-->>Inner Join
--Inner Join use to retvire data that present at both table and satisfy join condition.


SELECT * FROM DAY4.dbo.Employees_1
SELECT * FROM DAY4.dbo.Countries

-->>SELECT * FROM JobHistory
-- Display name and departname of emplyoee

SELECT Employees_1.FirstName , Employees.LastName AS 'Name' , Departments.DepartmentName FROM Employees 
JOIN Departments 
...
-->>Outer Join
--Outer join use to retrive data that present in both/all table or only in any of one.

-->>Left Join
--Left Join use to retrive data, present in both/all tables along with all data from Left side table.
...
-->>Self join
--When we perform joining on table with it self it refe as self-join.
...
-->>Cross Join
--Simillar to other joins Cross join also use to retrive combine data from tables, 
...
INSERT INTO Scanks(Snack ,Price)
VALUES('Burger',25),
('Sandwich',30),
('Samosa',20),
('Toast',15)
